...
export default function NovoOrcamentoPage() {
  ...
  function preencherMaterial(id: string) {
    ...
    if (m) setNovoItem(i => ({ ...i, descricao: m.nome, preco_unitario: m.preco_padrao || m.preco || m.preco_unitario || 0 }))
  }
  ...
  async function salvar() {
    ...
    try {
      ...
      if (itens.length > 0) {
        ...
        const { error: itensErr } = await supabase.from('orcamento_itens').insert(itensPayload)
              ...
              <div style={{ background: '#f9f7f3', border: '1px solid #EDE9E2', borderRadius: 10, padding: 16, marginBottom: 16 }}>
                ...
                <div className="form-row form-row-2">
                  ...
                  {novoItem.tipo === 'material' && materiais.length > 0 && (
                    <div className="form-group">
                      <label className="form-label">SELECIONAR DO ESTOQUE</label>
                ...
                <div className="form-row form-row-2">
                  <div className="form-group">
                    ...
                    <input className="form-input" type="number" min="0" step="0.01" value={novoItem.quantidade}
                      onChange={e => setNovoItem(i => ({ ...i, quantidade: parseFloat(e.target.value) || 0 }))} />
                  </div>
                  <div className="form-group">
                    ...
                    <input className="form-input" type="number" min="0" step="0.01" value={novoItem.preco_unitario}
                      onChange={e => setNovoItem(i => ({ ...i, preco_unitario: parseFloat(e.target.value) || 0 }))} />
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 700, color: 'var(--dark)' }}>Total: {fmt(calcTotal(novoItem))}</span>
                  <button className="btn btn-gold" onClick={adicionarItem}>+ Adicionar Item</button>
                </div>
              </div>
